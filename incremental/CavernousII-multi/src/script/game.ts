...
class Game {
	...
	runActions(time: number): number {
		...
		while (time > 0.001) {
			...
			if (
				zones[this.currentZone].queues.every(
					(q, i) => this.clones[i].isSyncing || this.clones[i].damage == Infinity || this.clones[i].notSyncing || !q.hasFutureSync(),
				) &&
	...
	mainLoop() {
		...
		if (settings.running) {
			if (mana.current == 0 || this.clones.every(c => c.damage === Infinity)) {
				...
				if (
					settings.autoRestart == AutoRestart.RestartAlways ||
					(settings.autoRestart == AutoRestart.RestartDone && this.clones.every(c => c.repeated))
				) {
					resetLoop();
				}
			} else {
		...
		if (
			...
			(settings.autoRestart == AutoRestart.WaitAll &&
				zones[this.currentZone].queues.every(q => !q.getNextAction()) &&
				this.clones.some(c => c.damage < Infinity)) ||
		...
		if (this.currentZone == this.displayZone) {
			...
			this.zoneTimeNode.innerText = writeNumber(
				Math.max(0, (zones[this.displayZone + 1]?.zoneStartTime || 0) - (zones[this.displayZone].zoneStartTime || 0)) / 1000,
				1,
			);
		}
